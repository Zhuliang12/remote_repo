...
def main():
    warnings.warn('this script is aimed to demonstrate how to convert the\n'
                  'JSON file to a single dataset, and not to handle\n'
                  'multiple JSON files to generate a real-use dataset.')
    parse = argparse.ArgumentParser()
    parse.add_argument('--json_file',default="D:/mydata/stervel/Seversatl/labels/voc/demo")
    parse.add_argument('-o', '--out', default=None)
    ...
    for i in range(len(count)):
        ...
        if os.path.isfile(path):
            ...
            if data['imageData']:
                ...
                imageData = os.path.join(os.path.dirname(path), data['imageData'])
