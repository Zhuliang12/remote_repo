...
"""把图片重新保存一遍，因为运行 YOLOv8 时报错： corrupt JPEG restored and saved。
   # noqa 特殊注释 可以用来告诉lint工具不要对其后面的代码进行检查 但需谨慎使用 """
...
def fix_images(original_images_path):
    ...
    print('Fixing images ...')
    with concurrent.futures.ThreadPoolExecutor(max_workers=None) as executor:  # noqa  这种方式只能实现 频繁实现一个小任务的情况
...
if __name__ == '__main__':
    train_path = "~/liuj/2023_06_07_connector/dataset_connector/images/train"
    val_path = "~/liuj/2023_06_07_connector/dataset_connector/images/validation"
    test_path = "~/liuj/2023_06_07_connector/dataset_connector/images/test"
    # images_paths = train_path, val_path, test_path

    # 单独修复某个文件夹内的图片
    images_paths = (r'/media/drin/PS2000/2023_06_07_connector/dataset_connector/original_data/'
                    r'2023-09-08--巡检车自动曝光，亮度 100/images'),
